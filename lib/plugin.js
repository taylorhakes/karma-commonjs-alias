...
var initCommonJS = function(/*config.files*/ files, config) {
  ...
  if(newFiles.length) {
    ...
    newFiles.forEach(function(file) {
      var filePath = path.join(tmpDir, file.pattern);
      fs.writeFileSync(filePath,
        getWrappedContent(fs.readFileSync(require.resolve(file.pattern)), file.pattern));
...
function getWrappedContent(content, path) {
  return 'window.__cjs_module__ = window.__cjs_module__ || {};' +
